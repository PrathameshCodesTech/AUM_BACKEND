...
class AdminPropertyListView(generics.ListAPIView):
    ...
    - status: Filter by status (draft, published, sold_out, archived)
    ...
    def get_queryset(self):
        queryset = Property.objects.all().select_related('developer').order_by('-created_at')
        ...
        if search:
            queryset = queryset.filter(
                ...
                Q(address__icontains=search) |  # 👈 Changed to address
...
class AdminPropertyDetailView(APIView):
    ...
    def get(self, request, property_id):
        try:
            property_obj = Property.objects.select_related('developer').get(id=property_id)
            serializer = AdminPropertyDetailSerializer(property_obj)
...
class AdminPropertyCreateView(APIView):
    ...
    def post(self, request):
        serializer = AdminPropertyCreateUpdateSerializer(data=request.data)
        ...
        try:
            ...
            logger.info(f"✅ Property created by admin {request.user.username}: {property_obj.name}")
            
            return Response({
                'success': True,
                'message': 'Property created successfully',
                'data': AdminPropertyDetailSerializer(property_obj).data
...
class AdminPropertyUpdateView(APIView):
    ...
    def put(self, request, property_id):
        ...
        try:
            ...
            logger.info(f"✅ Property updated by admin {request.user.username}: {property_obj.name}")
...
class AdminPropertyDeleteView(APIView):
    ...
    def delete(self, request, property_id):
        try:
            ...
            logger.info(f"✅ Property deleted by admin {request.user.username}: {property_obj.name}")
...
class AdminPropertyActionView(APIView):
    ...
    Perform actions on property (publish, unpublish, archive, feature)
    """
    permission_classes = [IsAuthenticated, IsAdmin]
    ...
    def post(self, request, property_id):
        ...
        if action == 'publish':
            ...
            message = f'Property "{property_obj.name}" published successfully'
            ...
            message = f'Property "{property_obj.name}" archived'
        ...
        logger.info(f"✅ Admin {request.user.username} performed '{action}' on property: {property_obj.name}")
...
class PropertyImageUploadView(APIView):
    ...
    def post(self, request, property_id):
        ...
        serializer = PropertyImageUploadSerializer(data=data)
        ...
        logger.info(f"✅ Image uploaded for property {property_obj.name} by {request.user.username}")
        
        return Response({
            'success': True,
            'message': 'Image uploaded successfully',
            'data': PropertyImageSerializer(image, context={'request': request}).data
        }, status=status.HTTP_201_CREATED)
...
class PropertyImageDeleteView(APIView):
    ...
    def delete(self, request, property_id, image_id):
        try:
            ...
            logger.info(f"✅ Image deleted from property {property_id} by {request.user.username}")
...
class PropertyDocumentListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        property_id = self.kwargs.get('property_id')
        return PropertyDocument.objects.filter(property_id=property_id).order_by('-created_at')
...
class PropertyDocumentUploadView(APIView):
    ...
    def post(self, request, property_id):
        ...
        serializer = PropertyDocumentUploadSerializer(data=data)
        ...
        logger.info(f"✅ Document uploaded for property {property_obj.name} by {request.user.username}")
        
        return Response({
            'success': True,
            'message': 'Document uploaded successfully',
            'data': PropertyDocumentSerializer(document, context={'request': request}).data
        }, status=status.HTTP_201_CREATED)
...
class PropertyDocumentDeleteView(APIView):
    ...
    def delete(self, request, property_id, document_id):
        try:
            document = PropertyDocument.objects.get(id=document_id, property_id=property_id)
            document.delete()
            
            logger.info(f"✅ Document deleted from property {property_id} by {request.user.username}")
...
class PropertyUnitListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        property_id = self.kwargs.get('property_id')
        return PropertyUnit.objects.filter(property_id=property_id).order_by('unit_number')
...
class PropertyUnitCreateView(APIView):
    ...
    def post(self, request, property_id):
        ...
        logger.info(f"✅ Unit created for property {property_obj.name} by {request.user.username}")
...
class PropertyUnitUpdateView(APIView):
    ...
    def put(self, request, property_id, unit_id):
        ...
        serializer = PropertyUnitCreateUpdateSerializer(unit, data=request.data, partial=True)
        ...
        logger.info(f"✅ Unit updated for property {property_id} by {request.user.username}")
...
class PropertyUnitDeleteView(APIView):
    ...
    def delete(self, request, property_id, unit_id):
        try:
            ...
            logger.info(f"✅ Unit deleted from property {property_id} by {request.user.username}")
