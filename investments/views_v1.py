...
class CreateInvestmentView(APIView):
    ...
    def post(self, request):
        ...
        logger.info(f"📥 Request data: {request.data}")
        logger.info(f"📥 Amount type: {type(request.data.get('amount'))}")
        logger.info(f"📥 Amount value: {repr(request.data.get('amount'))}")
        logger.info(f"📥 Units type: {type(request.data.get('units_count'))}")
        logger.info(f"📥 Units value: {repr(request.data.get('units_count'))}")
